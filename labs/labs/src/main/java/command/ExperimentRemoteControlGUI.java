...
public class ExperimentRemoteControlGUI extends JFrame {
	...
	public RemoteControlGUI() {
		...
		*/
		container.add(labelLight);
		ArrayList<JRadioButton> buttonsList = generateRadioButtons(new String[] {"On", "Off"}, new Command[] {new LightOnCommand(light), new LightOffCommand(light)}, light);
		ButtonGroup groupLight = generateButtonGroup(new String[] {"On", "Off"}, new Command[] {new LightOnCommand(light), new LightOffCommand(light)}, light);
		//gridLight.add()
		container.add(gridLight);
	...
	private ButtonGroup generateButtonGroup(String[] titles, Command[] commands, Device device) {
		ButtonGroup buttonGroup = new ButtonGroup();
		for (int i = 0; i < titles.length; i++) {
			buttonGroup.add(generateRadioButton(titles[i], commands[i], device));
		}
		return buttonGroup;
	}

	private JRadioButton generateRadioButton(String title, Command command, Device device) {
		...
		radioButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				updateState();
				statusBar.setText(device.toString());
			}
		});
	...
	private ArrayList<JRadioButton> generateRadioButtons(String[] titles, Command[] commands, Device device) {
		ArrayList<JRadioButton> radioButtons = new ArrayList<>();

		for (int i = 0; i < titles.length; i++) {
			radioButtons.add(generateRadioButton(titles[i], commands[i], device));
		}
